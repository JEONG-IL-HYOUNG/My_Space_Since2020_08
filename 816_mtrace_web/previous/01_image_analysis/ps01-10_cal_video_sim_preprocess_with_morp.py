# -*- coding: utf-8 -*-
# date : 200529
# goal : 원소스에서 데이터를 받아와서 전처리해서 DB에 올림
#
# desc : 1. source
# desc :    video_syc, video_key_tag, tag_syc
# desc : 2. target
# desc :    anla_morp : 비디오 별 메타정보 텍스트에서 형태소 분석한 결과 
# desc :      `video_id` varchar(200) 
# desc :      `youtube_id` varchar(100) 
# desc :      `MORP` varchar(100) - 리스트를 컬럼에 집어넣음 
...
print('ps01-10_cal_video_sim_preprocess_with_morp start')
...
for i in range(len(contents_df)):
    ...
    #key_tag_kor_token[i] = key_tag_kor_token_okt
    key_tag_kor_token.append(key_tag_kor_token_okt)
...
for i in range(len(contents_df)):
    ...
    #tag_kor_token[i] = tag_kor_token_okt
    tag_kor_token.append(tag_kor_token_okt)
...
for i in range(len(contents_df)):
    ...
    #title_kor_token[i] = title_kor_token_okt
    title_kor_token.append(title_kor_token_okt)
...
contents_fin["title_kor_noun"] = contents_df["title_kor_noun"].astype(str)
contents_fin["key_tag_kor_noun"] = contents_df["key_tag_kor_noun"].astype(str)
contents_fin["tag_kor_noun"] = contents_df["tag_kor_noun"].astype(str)
...
##ps insert tag
print("insert morp to mstuv DB ")

db = oaislib.fn_lab_db_connect()
try:
    with db.cursor() as cursor:

        ## delete old morp data
        sql = 'truncate table anal_morp'
        cursor.execute(sql)
        db.commit()                    
        ...
        for i in range(len(contents_df)):
            ...
            title_morp_ko_str = contents_df['title_kor_noun'].iloc[i]
            keytag_morp_ko_str = contents_df['key_tag_kor_noun'].iloc[i]
            tag_morp_ko_str = contents_df['tag_kor_noun'].iloc[i]
            
            sql = 'insert into anal_morp (video_id, youtube_id, TITLE_ORG, KEYTAG_ORG, TAG_ORG, TITLE_MORP_KO, KEYTAG_MORP_KO, TAG_MORP_KO,INSERT_DATE) VALUES ( "{}" , "{}", "{}", "{}", "{}", "{}", "{}", "{}", "{}")'.format(video_id_str, youtube_id_str, title_org_str, keytag_org_str, tag_org_str, title_morp_ko_str, keytag_morp_ko_str, tag_morp_ko_str, date_str)
            #print(sql)
            cursor.execute(sql)
        db.commit()
        
        #check update error
        sql_check = "SELECT COUNT(0) FROM anal_morp"
...
print('ps01-10_cal_video_sim_preprocess_with_morp finish')
