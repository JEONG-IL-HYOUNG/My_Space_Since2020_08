...
#
# prss : 01. parameter 
# prss : 02. data loading(anal_morp)
# prss : 03. vectorize
...
print('ps01-22_cal_video_sim_cosine_fast_with_morp.py start')
##ps_data loading(anal_morp)
db = oaislib.fn_lab_db_connect()
try:
    with db.cursor() as cursor:
        sql_select_anal_morp = ("SELECT VIDEO_ID, YOUTUBE_ID, TITLE_ORG, TITLE_MORP_KO, "  
                             "KEYTAG_MORP_KO, TAG_MORP_KO FROM anal_morp")
        cursor.execute(sql_select_anal_morp)
...
for i in range(num_sentence):
    ...
    video_id01 = morp_df['video_id'].iloc[i]
    youtube_id01 = morp_df['youtubeid'].iloc[i]
    title01 = morp_df['title'].iloc[i]
    ...
    if rlt_cosine_count > 0:
        ...
        for k in range(num_sim):
            ...
            video_id02 = morp_df['video_id'].iloc[rlt_index]
            youtube_id02 = morp_df['youtubeid'].iloc[rlt_index]
            title02 = morp_df['title'].iloc[rlt_index]
        ...
        try:
            with db.cursor() as cursor:
                ...
                sql_del_sim = 'delete from ANAL_VIDEO_SIMILARITY_WITH_MORP where VIDEO_ID1 = "' + video_id01 + '"'
...
print('ps01-22_cal_video_sim_cosine_fast_with_morp.py finish')            
