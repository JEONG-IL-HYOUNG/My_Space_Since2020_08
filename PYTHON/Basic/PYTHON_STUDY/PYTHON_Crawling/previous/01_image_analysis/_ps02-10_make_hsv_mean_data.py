...
def get_new_video_ids():
    ...
    sql_str02 = 'select video_id from anal_video_hsv_mean'
    ...
    for i in range(video_cnt): #video_cnt
        ...
        try:
            with db.cursor() as cursor:
                sql = f'select count(*) from anal_video_hsv_mean where video_id = "{video_id}"'
                cursor.execute(sql)
                result = cursor.fetchall()
                if result[0][0] == 0:
                    sql_update = ('insert into anal_video_hsv_mean (video_id) '
                                  'values("{}")'
                                  .format(video_id))
        ...
        if idx_download == 0:
            ...
            ##ps upload hsv to mstuv.anal_video_hsv_mean
            db = oaislib.fn_lab_db_connect()
            try:
                with db.cursor() as cursor:
                    sql_update = ('update anal_video_hsv_mean '
                                  'set youtube_id = "{}", '
                                  'success = "yes", '
                                  'h_m = "{}" , '
                                  's_m = "{}" , '
