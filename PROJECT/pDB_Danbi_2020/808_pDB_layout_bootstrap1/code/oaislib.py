...
#
# desc : 
# desc : oaislib.fn_remove_all_files_in_folder(folder_name):폴더내 파일 삭제 
# desc : oaislib.fn_read_utftxt_to_list(filename): utf8의 텍스트를 읽어 들여서 리스트로 리턴 
# desc : oaislib.fn_get_clean_text(text): 문자 추출 
# desc : oaislib.fn_get_timetail() : 파일명 생성을 위한 테일링
# desc : oaislib.fn_get_df_from_mstuv_dev_db(sql_str): 개발서버에서 sql결과를 df로 리턴
# desc : oaislib.fn_get_date_str(): YYYY-MM-DD의 날짜 문자열 리턴
# desc : oaislib.fn_remove_all_files_in_folder(folder_name):
...
## mlab 개발DB에 sql문 실행
def fn_run_sql_to_mstuv_dev_db(sql):
    db = fn_connect_dev_db()
    try:
