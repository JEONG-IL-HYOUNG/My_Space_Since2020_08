...
def return_dummy_df():
    # std_video_df, sim_video_df = search_video(sim_video)
    data1 = {'video_id' : [ "m7atGkba-Z8",
                            "6Q3uaepRCl4",
                            "qGjAWJ2zWWI",
                            "DQojZnhsvcE"],
             'title' : [ 'Tesla Cybertruck event in 5 minutes',
                         'Tesla CyberTruck Glass Failed',
                         "Agust D '대취타' MV",
                         "경찰 불렀더니 슈퍼카 4대가 왔습니다!"]}
    std_video_df = pd.DataFrame(data1)
    data2 = {'sim_video_id1' : ['RWZHR6CXFp8', 'VsGurWhsq7o','0rrMF0nJBks','c88Tvz16drg'],
             'title1' : [ 'Tesla CyberTruck Test Ride',
                          'Amazing Custom Cybertruck Designs',
                          'Tesla Cybertruck Unveil Starting at $39,900',
                          '2021 Tesla Cybertruck INTERIOR presentation'],
             'sim1': [90,
                             80,
                             70,
                             75],
             'sim_video_id2': ['Hw2Fa0dfTn0', 'EXMobCGwjC0', 'PnPcMAwcJJ0', 'd6UpHjJvnn0'],
             'title2': ['Tesla CyberTruck Armor Glass FAILED?',
                        'Military Tesla Cybertruck: Combat Vehicle',
                        'OMG is this why the Tesla Cybertruck',
                        "'Armor glass' smashes in Tesla truck demo"],
             'sim2': [80,
                             70,
                             90,
                             93],
             'sim_video_id3': ['ZclFfZGMZPU', 'wz_tClKRu8g', 'Pbz__2czqjA', 'Vwcjjfxphz4'],
             'title3': ["[1 시간 / 1 HOUR LOOP] Agust D",
                        "Agust D '대취타' MV (Group Reaction)",
                        "Agust D Daechwita Lyrics",
                        "Agust D 대취타 해외반응 한글자막 (BTS 슈가)"],
             'sim3': [90,
                             75,
                             90,
                             70],
             'sim_video_id4': ['P7N4cFKqLF4', 'Y9uZ8Qco0hY', 'GAj4xF_7agY', 'bHGhoQsTqw4'],
             'title4': ["클라스가 다른 두바이 경찰차들",
                        "오일머니의 끝판왕! 두바이 찐부자들",
                        "10억 미만 페라리는 출입불가!",
                        "슈퍼카 vs 경찰 바이크 과연 누가 더 빠를까?"],
             'sim4': [85,
                             82,
                             65,
                             70]
             }
    sim_video_df=pd.DataFrame(data2)
    cols=['sim1', 'sim2', 'sim3','sim4']
    sim_video_df[cols]=sim_video_df[cols].applymap('{:,.2f}'.format)

    return std_video_df, sim_video_df
...
def imgbase_show_all_info(img_dis, tag_sim_para):
    ...
    std_sim_video_df = imgbase_show_all(img_dis, tag_sim_para)
