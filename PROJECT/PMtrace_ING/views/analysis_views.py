    ...
    if request.method == 'POST':
        ...
        if error is None:
            ...
            if expr.match(input_yurl):
                ...
                encode = urllib.parse.quote_plus(input_yurl)

                target_url = 'https://www.youtube.com/results?search_query=' + encode
                ...
                try:
                    ...
                    youtube_ids = youtube_ids[:5]
                    for i in youtube_ids:
                        print(i)

                    pattern = '"viewCountText":{"simpleText":"조회수(.*?)"}'
                    result_cnt = re.findall(pattern, str(soup), re.MULTILINE | re.IGNORECASE)
                    result_cnt = result_cnt[:5]
                    for x in result_cnt:
                        print(x)

                    pattern = '"title":{"runs":(.*?)}'
                    result_title = re.findall(pattern, str(soup), re.MULTILINE | re.IGNORECASE)
                    result_title = [x[10:-1] for x in result_title]
                    for y in result_title[:3]:
                        print(y)

                    for i, x, y in zip(youtube_ids, result_cnt, result_title):
                        print(i, x, y)
